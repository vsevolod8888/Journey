...
class SettingsDataStore @Inject constructor(private val dataStore: DataStore<Preferences>) {
    ...
    private object PreferencesKeys {
        ...
        val KEY_THEME_MANUALLY_SET = booleanPreferencesKey("key_theme_manually_set")
    }
    ...
    val isThemeManuallySet: Flow<Boolean> = dataStore.data
        .map { preferences -> preferences[PreferencesKeys.KEY_THEME_MANUALLY_SET] ?: false }
        .distinctUntilChanged()

    suspend fun saveIsThemeDarkDataStore(value: Boolean) {
        Log.d("vvv", "theme $value")
        dataStore.edit { preferences ->
            preferences[PreferencesKeys.KEY_THEME] = value
        }
    }

    suspend fun markThemeManuallySet() {
        dataStore.edit { preferences ->
            preferences[PreferencesKeys.KEY_THEME_MANUALLY_SET] = true
        }
    }
    companion object{
